...
import { createArtist, getAllArtist } from "./FetchApi";
...
const AddArtistModal = (props) => {
  ...
  const submitForm = async (e) => {
    ...
    if (!fData.cImage) {
      return setFdata({ ...fData, error: "Please upload a Artist image" });
    ...
    try {
      let responseData = await createArtist(fData);
  ...
  return (
    <Fragment>
      {/* Black Overlay */}
      <div
        onClick={(e) => dispatch({ type: "addArtistModal", payload: false })}
        className={`${
          data.addArtistModal ? "" : "hidden"
        ...
        <div className="relative bg-white w-12/12 md:w-3/6 shadow-lg flex flex-col items-center space-y-4  overflow-y-auto px-4 py-4 md:px-8">
          <div className="flex items-center justify-between w-full pt-4">
            ...
            <span
              ...
              onClick={(e) =>
                dispatch({ type: "addArtistModal", payload: false })
              }
          ...
          <form className="w-full" onSubmit={(e) => submitForm(e)}>
            ...
            <div className="flex flex-col space-y-1 w-full">
              <label htmlFor="name">Artist Image</label>
              <input
                accept=".jpg, .jpeg, .png"
                onChange={(e) => {
                  setFdata({
